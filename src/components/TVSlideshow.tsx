        ...
        <div className="w-full max-w-7xl animate-fade-in-up">
          ...
          {currentSlide.type === "promo" && (
            <div className="flex flex-col items-center justify-center w-full h-full text-center">
              <div className="inline-flex items-center gap-3 px-6 py-2 bg-purple-500/20 text-purple-400 rounded-full font-bold uppercase tracking-widest border border-purple-500/30 mb-8 animate-pulse">
                Iscrizioni Aperte
              </div>
              <h2 className="text-7xl font-black uppercase tracking-tight text-transparent bg-clip-text bg-gradient-to-br from-white to-slate-400 mb-12">
                {currentSlide.tournament.name}
              </h2>
              
              <div className="grid grid-cols-3 gap-8 w-full max-w-5xl mb-16">
                {currentSlide.tournament.startDate && (
                  <div className="bg-slate-900/50 p-8 rounded-3xl border border-slate-800 flex flex-col items-center gap-4">
                    <Calendar className="w-12 h-12 text-blue-400" />
                    <span className="text-2xl font-bold">{new Date(currentSlide.tournament.startDate).toLocaleDateString('it-IT')}</span>
                  </div>
                )}
                <div className="bg-slate-900/50 p-8 rounded-3xl border border-slate-800 flex flex-col items-center gap-4">
                  <Banknote className="w-12 h-12 text-emerald-400" />
                  <span className="text-2xl font-bold">{currentSlide.tournament.pricePerPlayer || "Gratis"} €</span>
                </div>
                <div className="bg-slate-900/50 p-8 rounded-3xl border border-slate-800 flex flex-col items-center gap-4">
                  <Users className="w-12 h-12 text-purple-400" />
                  <span className="text-2xl font-bold">{currentSlide.tournament.registrations?.length || 0} Iscritti</span>
                </div>
              </div>

...
import { AlertTriangle, RotateCcw, Home, Swords } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-6 text-center">
      ...
      <p className="text-slate-400 max-w-lg mb-4 text-sm sm:text-base">
        {error.message || "Non è stato possibile caricare questa pagina o completare l'operazione."}
      </p>
      {error.digest && (
        <p className="text-xs font-mono text-slate-500 bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-lg mb-6">
          Codice Digest: {error.digest}
        </p>
      )}
